...
function Page({ params }) {
    ...
    const fetchProjects = async () => {
        try {
            const response = await axios.get(`../../../../../api/images/attribution/oneAttribution/${id}/${attributionId}`);
            ...
            setInitialValues({
                ...
                attribution: [
                    {
                        author: records.attribution.author || "",
                        authorDates: {
                            birthYear: records.attribution.authorDates?.birthYear || "",
                            deathYear: records.attribution.authorDates?.deathYear || "",
                            birthCity: records.attribution.authorDates?.birthCity || "",
                            deathCity: records.attribution.authorDates?.deathCity || "",
                        },
                        school: records.attribution.school || "",
                        centuryOfActivity: records.attribution.centuryOfActivity || ""
                    },
                ],
            });
        } catch (error) {
    ...
    const handleSubmit = async (values) => {
        try {
            ...
            const response = await axios.put(`../../../../../../api/images/attribution/oneAttribution/${id}/${attributionId}`, jsonData);
                ...
                <main className="flex-1 p-3 pt-4 h-screen bg-white overflow-y-auto z-1 lg:ml-64 lg:mt-20 "  style={{ fontFamily: "calibri" }}>
                    <div className="mb-6 mt-4 ml-6">
                        ...
                        <p className="" style={{ fontSize: "14px" }}>
                            <Link href={"../image/list"}>{title}</Link> / Update Attribution
                        </p>
                    </div>
                    ...
                    >
                        {({ values, isSubmitting }) => (
                            <Form name="employeeForm" id="employeeForm" className="mx-7" method="post">
                                ...
                                <div className="flex items-center justify-end">
                                    <button
                                        type="submit"
                                        className="bg-gray-700 hover:bg-gray-900 text-white py-2 px-6 rounded focus:outline-none focus:shadow-outline"
